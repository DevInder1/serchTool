...
def _select_providers(req: SearchRequest) -> list[Provider]:
    active = [p for p in _ALL_PROVIDERS if p.enabled]
    if req.sources:
...
def providers() -> dict:
    return {
        "providers": [
            {"name": p.name, "enabled": p.enabled} for p in _ALL_PROVIDERS
        ]
    }
...
async def search(req: SearchRequest) -> SearchResponse:
    ...
    selected = _select_providers(req)
    errors: dict[str, str] = {}
